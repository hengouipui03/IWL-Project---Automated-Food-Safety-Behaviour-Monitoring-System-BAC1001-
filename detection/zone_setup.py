...
ZONE_TYPES = {
    ...
    "4": {"key": "water_stream",   "label": "WATER STREAM",    "color": (0, 255, 255)},
}

ZONE_BY_KEY = {v["key"]: v for v in ZONE_TYPES.values()}


def get_zone_color(zone_type):
    return ZONE_BY_KEY.get(zone_type, {"color": (255, 255, 255)})["color"]


def get_zone_label(zone_type):
    return ZONE_BY_KEY.get(zone_type, {"label": zone_type})["label"]
...
camera_source = parse_camera_source(
    ...
    else existing_config.get("camera_source", 0)
)
...
min_wash_duration = existing_config.get("min_wash_duration", 20)
...
zones = existing_config.get("zones", default_zones)

# Make sure old config files also get the new water_stream key
for key in default_zones:
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing:
            ...
            zones.setdefault(active_type, [])
            zones[active_type].append({
                "x1": x1,
                "y1": y1,
                "x2": x2,
                "y2": y2
            ...
            print(
                f"✔ {active_type} #{len(zones[active_type])} saved: "
                f"({x1},{y1}) → ({x2},{y2})"
            )
...
print("  4 — Draw WATER STREAM zone")
...
while True:
    ...
    if drawing and active_type:
        color = get_zone_color(active_type)
        cv2.rectangle(
            display,
            (start_x, start_y),
            (temp_end_x, temp_end_y),
            color,
            2
        )
    ...
    # HUD
    if active_type:
        label = get_zone_label(active_type)
        cv2.putText(
            display,
            f"Drawing: {label}",
            (10, 30),
        ...
        cv2.putText(
            display,
            "Press 1, 2, 3, or 4 to select zone type",
            (10, 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (200, 200, 200),
    ...
    if key == ord("1"):
        ...
        active_type = "water_stream"
        print("Mode: WATER STREAM")

    elif key == ord("u") and active_type and zones.get(active_type):
        removed = zones[active_type].pop()
        print(f"✖ Removed last {active_type}: {removed}")
...
if save_flag:
    ...
    site_prompt = (
        f"Enter site name (e.g. Factory A - Sink Row 1)"
        f"{f' [{site_name}]' if site_name else ''}: "
    )
    ...
    camera_prompt = (
        f"Enter camera ID (e.g. camera_01)"
        f"{f' [{camera_id}]' if camera_id else ''}: "
    )
    ...
    config = {
        ...
        "min_wash_duration": min_wash_duration,
        "zones": zones
    }
